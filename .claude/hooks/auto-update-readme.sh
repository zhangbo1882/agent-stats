...
if [ "$stop_hook_active" = "true" ]; then
  printf '{"decision": "allow", "reason": "Hook already active, allowing stop"}'
  exit 0
fi
...
if [ "$CHANGED_FILES" -eq 0 ]; then
  ...
  printf '{"decision": "allow", "reason": "No changes detected"}'
  exit 0
fi

# Has changes, suggest Claude update README
printf '{"decision": "continue", "reason": "Detected %s file changes. Consider using /auto-update-readme skill to update README.md."}' "$CHANGED_FILES"
